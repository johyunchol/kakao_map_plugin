...
class KakaoMapWeb extends StatefulWidget {
  final MapWebCreateCallback? onMapCreated;
...
class _KakaoMapWebState extends State<KakaoMapWeb> {
  late final KakaoMapWebController _mapController;
  final GlobalKey webViewKey = GlobalKey();
  final WebUri myUrl = WebUri("https://velog.io/@jhkim0122");
  ...
  @override
  void dispose() {
    _messageSubscription?.cancel(); // ✅ 구독 해제
    print('>>>> dispose 하면서 구독 해제');
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return InAppWebView(
      ...
      // initialUrlRequest: URLRequest(url: WebUri(AuthRepository.instance.baseUrl ?? '')),
      // initialFile: "assets/html/sample.html",
      initialData: InAppWebViewInitialData(data: '''<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
    ...
    window.onload = function () {
        ...
        if (${widget.onMapTap != null}) {
            ...
            kakao.maps.event.addListener(map, 'click', function (mouseEvent) {
              ...
              window.parent.postMessage("onMapCreated:"+JSON.stringify(clickLatLng), "*");
